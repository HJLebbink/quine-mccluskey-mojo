...
struct Checked[N_BITS: Int]:
    ...
    fn at(self, bit_count: Int) -> DTypePointer[DType.bool]:
        debug_assert(
            bit_count < N_BITS + 1, "Checked:at: position 'bit_count' out of range"
        )
...
fn reduce_qm[
    ...
    for i in range(len(minterms_input)):
        minterms.add[CHECK_CONTAINS=False, SHOW_INFO=SHOW_INFO](minterms_input.data[i])
    minterms.sort()
    ...
    return petrick_simplify[T, T, N_BITS, SHOW_INFO](minterms.to_set(), minterms_input)
